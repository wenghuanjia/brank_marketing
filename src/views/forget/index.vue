...
export default {
  ...
  setup() {
    ...
    const onSubmit = (values) => {
      ...
      handleForgetPwd(data, loading, handleBack);
    };
    const handleBack = () => {
      router.push({
        path: "/login",
...
// 注册
async function handleForgetPwd(data, loading, handleBack) {
  if (loading.value) return;
  try {
    let res = await forgetPwd(qs.stringify(data));
    Toast(res.msg);
    if (res.status === 1) {
